...
{% include image.html src="cg/vertices_to_fragments/image_Cohen-Sutherland-Clipping.jpg" caption="图片来自《交互式计算机图形学》E文版" width="290" height="237" align="center" %}
...
{% include image.html src="cg/vertices_to_fragments/image_Liang-Barsky-Clipping0.jpg" caption="图一" width="290" height="237" align="center" %}
{% include image.html src="cg/vertices_to_fragments/image_Liang-Barsky-Clipping1.jpg" caption="图二" width="290" height="237" align="center" %}
...
{% include image.html src="cg/vertices_to_fragments/image_Sutherland-Hodgeman-Clipping.jpg" caption="图片来自《交互式计算机图形学》E文版" width="290" height="237" align="center" %}
...
{% include image.html src="cg/vertices_to_fragments/image_DDA.jpg" caption="图片来自《交互式计算机图形学》E文版" width="290" height="237" align="center" %}
...
{% include image.html src="cg/vertices_to_fragments/image_bresenham0.jpg" caption="d" width="290" height="237" align="center" %}
{% include image.html src="cg/vertices_to_fragments/image_bresenham1.jpg" caption="d" width="290" height="237" align="center" %}
...
{% include image.html src="/img/cg/vertices_to_fragments/image_bresenham10.jpg" caption="图十 误差累计0.3" width=250 align="right" %}
...
{% include image.html src="/img/cg/vertices_to_fragments/image_bresenham12.jpg" caption="图十一 误差0.6超过了0.5，需要修正得到-0.4" width=250 align="right" %}
紧接着x值继续增加1，y历史变化0.3，此时再加上斜率0.3为0.6，比较后发现超出了0.5，于是全新的点(2,0.6)属于像素(2,1)，如图十一所示。此时y的历史变化值0.6就需要进行一次修正，因为0.6是针对像素(2,0)而言的，也就是说：以像素(2,0)来说，因变量的0.6表示向上增长了0.6个单位，但对于像素(2,1)而言，此时的因变量应该为0.6-1=-0.4，表示图中点M的y值相对于像素(2,1)的中心点在下方0.4个单位。如果继续增长想“逃脱”第二行像素(x,1)的范围，至少还需要0.5+(0.4)=0.9个单位。

{% include image.html src="/img/cg/vertices_to_fragments/image_bresenham14.jpg" caption="图十二 误差累计-0.1" width=250 align="left" %}
{{site.b}}修正就是减去1，从而y的史变化值成了-0.4，后面依然还是循环（直到线段的另一个端点结束），从图十四中知道点Q的历史变化y为0.2+0.3=0.5，此前约定像素的范围得知点Q属于像素(5,2),修正后为-0.5。循环继续，x继续增加1，y的历史变化值继续加上斜率0.3，变成了-0.1。比较后小于0.5，说明垂直像素不做变化，如图十三。
...
测试截图：
{% include image.html src="cg/vertices_to_fragments/image_bresenham_screenshot.jpg" caption="测试截图" width=250 align="right" %}
